...
Uso:
  ...
  perl asr_system.pl process DIR
...
sub extract_text_from_html {
    ...
    # Meta descripción como último recurso
...
sub classify_errors {
    ...
    my @classified_errors;
    
    foreach my $segment (@$timed_segments) {
        my $align_info = $segment->{alignment_info};
        my $error_label = classify_single_error($align_info);
        
        if ($error_label ne '<M>') {
            push @classified_errors, {
                start_time => $segment->{start_time},
                end_time => $segment->{end_time},
                transcription_word => $align_info->{word1} || '',
                ipu_word => $align_info->{word2} || '',
                error_label => $error_label,
                operation => $align_info->{operation}
            };
        }
    }
...
sub classify_single_error {
    my ($align_info) = @_;
    
    my $operation = $align_info->{operation};
    my $word1 = $align_info->{word1} || '';
    my $word2 = $align_info->{word2} || '';
...
sub create_textgrid_tiers {
    my ($textgrid, $timed_segments, $classified_errors) = @_;
    
    
    # Tier Transc
    my $transc_tier = new NodeChain('Transc');
    foreach my $segment (@$timed_segments) {
        ...
        $transc_tier->addNodes($node);
    }
    $textgrid->addTier($transc_tier);
    
    # Tier TranscErrors
    my $errors_tier = new NodeChain('TranscErrors');
    foreach my $error (@$classified_errors) {
        my $error_text = sprintf("%s: '%s' vs '%s'", 
                                $error->{error_label},
                                $error->{transcription_word},
                                $error->{ipu_word});
...
sub process_dataset {
    ...
    # Clasificar errores
    my $classified_errors = classify_errors($timed_segments);
    
    # Crear tiers en TextGrid
    create_textgrid_tiers($textgrid, $timed_segments, $classified_errors);
    ...
    my $results = {
        ...
        total_errors => scalar(@$classified_errors),
